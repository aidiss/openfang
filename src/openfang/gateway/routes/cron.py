...
from openfang.gateway.deps import CronDataDep, DepsDep, get_cron_data, templates
...
@router.post("")
async def cron_create(request: Request, deps: DepsDep):
    """Create a cron job."""
    ...
    job = await deps.scheduling.cron.create(schedule, task)

    if is_htmx(request):
        jobs = await get_cron_data(deps)
...
async def cron_delete(request: Request, job_id: str, deps: DepsDep):
    """Delete a cron job."""
    await deps.scheduling.cron.delete(job_id)
