...
async def heartbeat_status(gateway: GatewayDep):
    ...
    return {
        ...
        "last_alert": gateway.last_heartbeat_alert,
    }


@router.post("/run")
async def heartbeat_run(request: Request, gateway: GatewayDep):
    """Trigger immediate heartbeat check."""
    result = await run_heartbeat(gateway)
